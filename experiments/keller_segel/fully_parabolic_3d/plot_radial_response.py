...
arms now share chi=1 and differ only in initial mass M (effective coupling F=M), so
the arms are keyed by LABEL (weak / delayed), not chi.  Panels:
...
def load(run_dir):
    ...
    for d in sorted(glob.glob(os.path.join(run_dir, "*_chi*_N*_K*_tau*_seed*"))):
        ...
        # M from manifest (dir name has no M); fall back to NaN
        Mval = np.nan
...
def seedmean(runs, sel, col):
    grp = [r for r in runs if all(r[k] == v for k, v in sel.items()) and col in r["cols"]]
    if not grp:
...
def main():
    ...
    ap.add_argument("--baseN", type=int, default=100000)
    ap.add_argument("--K", type=int, default=None, help="primary bandwidth (default: max present)")
    ap.add_argument("--out_prefix", default="figureB_radial_response")
    args = ap.parse_args()
    runs = load(args.run_dir)
    if not runs:
        raise SystemExit("no radial runs found")
    labels = sorted(set(r["label"] for r in runs))
    K = args.K if args.K is not None else max(r["K"] for r in runs)
    tau0 = sorted(set(r["tau"] for r in runs))[0]
    Md = next((r["M"] for r in runs if r["label"] == "delayed"), np.nan)
    Mw = next((r["M"] for r in runs if r["label"] == "weak"), np.nan)
    sd = dict(label="delayed", N=args.baseN, K=K, tau=tau0)
    sw = dict(label="weak", N=args.baseN, K=K, tau=tau0)
    Ns = sorted(set(r["N"] for r in runs if r["label"] == "delayed" and r["K"] == K))
    nseed = len(set(r["seed"] for r in runs if r["label"] == "delayed"
                    and r["N"] == args.baseN and r["K"] == K))

    fig, ax = plt.subplots(1, 4, figsize=(17, 3.9))
    # (a) M_v & exact + G_v (delayed)
    ...
    # (d) N-refinement R_0.5 (delayed)
    for N in Ns:
        t, m, _ = seedmean(runs, dict(label="delayed", N=N, K=K, tau=tau0), "R_0_5")
        if t is not None:
    ...
    fig.suptitle(f"Figure B: 3D fully-parabolic radial delayed chemical response "
                 f"(reconstruction-free $R_q$; K={K}, {nseed} seeds)", fontsize=12)
    ...
    if m5 is not None:
        ...
        print(f"delayed M={Md:g} K={K}: R0.5 {R0:.4f}->{m5[-1]:.4f} (ratio {m5[-1]/R0:.3f}), "
              f"t_turn={tturn:.2f}, t_focus10={foc[0] if foc.size else 'none'}")
    tw, mw, _ = seedmean(runs, sw, "R_0_5")
    if mw is not None:
        print(f"weak    M={Mw:g} K={K}: R0.5 {mw[0]:.4f}->{mw[-1]:.4f} (ratio {mw[-1]/mw[0]:.3f})")
