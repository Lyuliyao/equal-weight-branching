...
#   allen_cahn/case2/functional_hierarchical_tensor_fourier.py
# Only difference vs the upstream reference: update_eval_msg_masked and
# evaluate_marginal are extended so that GHOST leaves (used to pad d_true up to
# 2**L, e.g. d=6 -> pad 8, L=3) are always marginalized via their constant-mode
# projection  sqrt(2)*c[node][0,:]  instead of emitting the sentinel 'None'.
# This lets the middle/root einsums stay unchanged while correctly integrating
# out the ghost dimensions, which is required for the d=6 experiment.
# The upstream file under allen_cahn/case2 is NOT modified.
# =============================================================================
import numpy as np
...
class FunctionalHierarchicalTensorFourier:
    ...
    def evaluate_marginal(self, x_n, mask):
        ...
        for l in reversed(range(0, self.L + 1)):
            for k in range(1, 2 ** l + 1):
                # print(eval_msg.keys())
                node = (k, l)
                if l == self.L and (k in mask or k in self.ghost_pt):
                    # marginalize this leaf (either user-requested or a ghost dim)
    ...
    def update_eval_msg_masked(self, N, node, eval_msg):
        k, l = node
        self.check_node(k, l)
        # For BOTH ghost and marginalized real leaves we emit the constant-mode
        # projection  sqrt(2) * c[node][0, :]  broadcast over the batch.  This is
        # the integral of the leaf basis (the constant Fourier mode picks up the
        # sqrt(2) normalization used in fourier_function), so middle/root einsums
        # need no special-casing and ghost subtrees are correctly marginalized.
