...
- **Decision:** a **genuine improvement over `t_b`** AND a **within-uncertainty cross-method agreement** — reconstruction-light, stable + resolution-convergent on **both** methods; the particle seed-bootstrap CI overlaps the LDG value and the literature blow-up (~1.21e-4). Caveats kept: 4/12 LDG fits (tight + secondary-confirmed), weak `q=0.1`, wide particle CI. Still use the **§11 limited wording** — report `T_core` as a concentration-time diagnostic, **no** continuum blow-up time — pending the `q_window`/`K`/`τ` sensitivity (Exp. B–D) to confirm the residual ~4% offset is reconstruction-controlled. Candidate for **§5.5 supporting diagnostic**.
