...
- **🟢 LDG gate PASSES (resolution-convergent + internally stable):** `T_core` over N=80→160→320 = 2.46e-4 → 1.234e-4 → **1.215e-4**; at N=320 all 12 `q×window` fits valid, `rel_spread=0.07<0.25`, and `T_core≈T_L2(1.219e-4)≈T_peak(1.211e-4)` all agree **and** match the literature LDG numerical blow-up ~1.21e-4. Coarse N is grid-floor-limited (`R_0.1<dx` by `t~1.2e-4`), as expected.
- **🟢/🟡 Particle stable + same scale, ~8% offset:** particle `current_fourier` `T_core` over Np=8e4→3.2e5 = 1.505e-4 → **1.318e-4** (stable, `rel_spread=0.055`), on the **same scale** and **within ~8%** of LDG. **But** the strict §12 within-uncertainty agreement is **not** met — LDG `[1.13,1.22]` and particle `[1.26,1.33]` bands are disjoint; the particle is systematically higher, consistent with the `K=10` Fourier drift slightly under-resolving the inner core (cf. §3.8).
- **Decision:** a **genuine improvement over `t_b`** — reconstruction-light, stable + resolution-convergent on **both** methods, agreeing to ~8% with each other and with the LDG blow-up scale. The strict cross-method gate fails by ~8%, so use the **limited wording** (§11): report `T_core` as a concentration-time diagnostic; **do not** quote a continuum blow-up time. Candidate for **§5.5 as a supporting diagnostic**, not a blow-up-time claim.
- **Data:** `experiments/keller_segel/core_collapse_time/{core_radii.py,fit_core_collapse.py,plot_core_collapse.py,run_ldg_core_radii.sb,run_particle_core_radii.sb}`, `ldg_reference/run_ldg.py --core_radii`, `ldg_comparison/simulation.py` (qs incl. R_0.05,R_0.3), `reference_results/keller_segel_ldg_pp/core_collapse_20260615_0053_af781af/` (ldg/particle R_q CSVs + `core_fit_*` + 4 figures + README).
