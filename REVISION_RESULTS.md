...
- **🟢 Physical-space blob residual (Form-I-smooth) — implemented, verified, swept (the principled Scenario-C operator):** instead of the high-Kl local *spectrum* (whose ∇ amplifies high-mode noise ~Kl), the local high part is an `η_h` **Gaussian blob** (KDE) — `v̂ = v_lo + χ(η_h*μ_v − η_h*(v_lo dx))`, the signed residual with the low-blob subtracted to avoid double-counting. `BlobResidualVField` (FFT-grid production path + exact kernel-sum verification): FD-verified to **4.4e-9**, no-double-counting to **1e-16**, grid-vs-exact **1.1%**, **5/5** unit tests pass. Bandwidth analysis: the plan's spacing rule `h~R₀.₈/√N₀.₈` is ~25× finer than `L/Kl` → noisy + expensive; the **smoother-than-Kl** regime uses `h=c_h·L` (`frac_L` rule).
- **🟢 Blob sweep result (`sf_blob_20260614_2010_84e6111`, 6 cfg × 4 seeds, N=8e4, corrected dual-CFL):** with the **real** solver CFL (`drift_cfl_solver_field`), the smoother blobs `c_h=0.06,0.09` recover/exceed the global-`K` abort time (1.95e-4, 1.99e-4 vs 1.91e-4) **and** have a genuinely **smaller real solver CFL (≈3.5)** than both global-`K` (4.45) **and** the spectral hybrid (3.63). The spacing-rule blob aborts almost immediately (`t≈2e-5`) — confirming the bandwidth analysis (shot noise). **The fix changes the reading:** the spectral hybrid's *true* solver CFL is **3.63** (not the 2.63 the old code logged — confirms the withdrawn claim); and blob `c_h=0.09`'s Fourier diag (5.22) ≫ its solver CFL (3.53), i.e. the blob makes a **tighter core with a smoother drift** (the old single-column code would have mislabeled it spiky). Figure: `figure_blob_sweep` panel (b).
- **🔴 honest catch persists (Q3):** reconstruction-free `R_0.2` at `t=1e-4` is within ~4× seed noise across all configs; the blob's residual energy is small (1–3%) and *shrinks* as it smooths (the stable `c_h=0.09` adds <1%). No demonstrable improvement in pre-singular tracked concentration.
- **🟢 Definitive accuracy test — LDG-style `t_b` (`solver_field_tb_20260614_2256_0293dd7`, 3 cfg × 2 res × 4 seeds):** the decisive comparison uses the **same LDG-matched particle resolution-gap time** as the particle method/LDG reference, not abort time: `S^DG_{Np,n}=‖Π_n^{P1DG}μ^u_{Np}‖_{L2}` (DG cross estimator), `t_b=inf{t: S̄_{(4Np,2n)}/S̄_{(Np,n)}≥1.05` held 5e-6`}`, main pair **(8e4,80)→(3.2e5,160)**, 1000× seed bootstrap CI. Result: **all three solver fields give `t_b` on the LDG scale and statistically indistinguishable** — `current_fourier` 8.5e-5 [7.3,10.7], `blob c_h=0.06` 8.2e-5 [6.9,10.7], `blob c_h=0.09` 9.6e-5 [7.9,10.3] (×1e-5; LDG ref 5.95–8.43e-5). **Reconstruction-free core radii `R_0.2,R_0.5` collapse on top of each other** across all three → the concentration *dynamics* are identical; the blob does **not** oversmooth or over-concentrate. The blob's *only* measurable effect is **stability** (`c_h=0.06` survives to `T` in 3/4 of the 3.2e5 seeds vs 1/4 for global-`K`, smallest real solver CFL 3.18).
- **Decision — Scenario A (final, definitive):** across the taper, blob, and `t_b` sweeps the conclusion is consistent and now settled on the **same metric as the paper's LDG comparison**: a solver-level residual blob/KDE drift can be inserted stably and **reduces the real solver-CFL / improves drift smoothness**, but it does **not** change the LDG-style concentration proxy `t_b` (CIs overlap) nor the reconstruction-free core radii. It is **not a validated accuracy improvement** on this benchmark → **record/appendix only, not in §5.4**. The **paper-safe result stands unchanged**: the current particle method + LDG-matched DG readout gives a particle resolution-gap time on the same scale as fixed-flux LDG at adequate particle count. Do not claim any solver field here resolves near-blow-up dynamics or a continuum blow-up time.
- **Data:** `experiments/keller_segel/ldg_comparison/{hybrid_vfield.py,blob_residual_vfield.py (FD-verified),test_blob_residual_vfield.py,analyze_solver_field.py,analyze_taper_sweep.py,analyze_blob_sweep.py,analyze_solver_field_tb.py,plot_taper_sweep.py,plot_blob_sweep.py,plot_solver_field_tb.py,run_solver_field_blob_sweep.sb,run_solver_field_tb_sweep.sb}`, `simulation.py --solver_field {current_fourier,two_level_spectral_residual,two_level_blob_residual}` (now logs `drift_cfl_solver_field` + `drift_cfl_fourier_diag` + `abort_diagnostics.json`), `reference_results/keller_segel_ldg_pp/{solver_field_sweep_<run_id>/,sf_taper_20260614_1983_3c6ae64/,sf_blob_20260614_2010_84e6111/,solver_field_tb_20260614_2256_0293dd7/}` (diag CSVs + summary/compare json + figures + plot_data). **Not in paper.**

### 3.9 🟢 Core-collapse time `T_core` from mass-quantile radii (candidate §5.5 supporting diagnostic)
- **Metric:** a **reconstruction-light** concentration-time proxy, method-independent: `R_q(t)=inf{r: μ(B(x_c,r))≥qM}`, fit `R_q²=α−βt`, `T_q=α/β`, aggregated over `q∈{0.1,0.2,0.3}` and 4 late windows → `T_core` median + `[p10,p90]`. Identical definition for LDG (sub-cell Gauss quadrature of the P1-DG field, mass-exact) and particles (ordered cloud distances). Secondary: `T_L2` from `S^{-2}`, `T_peak` from `peak^{-1}`. Stronger than the LDG-style `t_b` (§3.x) because it tracks the core *length scale* directly, not a resolution-gap.
- **🟢 LDG gate PASSES (resolution-convergent):** `T_core` over N=80→160→320 = 2.46e-4 → 1.234e-4 → **1.215e-4**; at N=320 the accepted fits have `rel_spread=0.073<0.25` and `T_core≈T_L2(1.219e-4)≈T_peak(1.211e-4)` agree **and** match the literature LDG blow-up ~1.21e-4. **Honest fit count: only 4 of 12** `q×window` `R_q²` fits pass the strict linear gate (the rest fail on curvature / grid floor — mainly `q=0.1` + early windows); the 4 accepted cluster tightly and the two independent secondary extrapolations confirm. *(An earlier note wrongly said "12/12"; corrected.)*
- **🟢 Particle agrees within uncertainty (after the seed-handling repair):** with **common-grid seed interpolation + a 1000× seed bootstrap** (Experiment A fix), particle `current_fourier` `T_core` over Np=8e4→3.2e5 = 1.505e-4 → **1.29e-4** (boot median 1.259e-4), and its seed-bootstrap CI **[1.11,1.36]e-4 OVERLAPS the LDG estimate and contains 1.215e-4** (point estimates agree to ~4%). The earlier "disjoint bands, ~8% offset" was a **bug** (seed-averaging silently dropped seeds whose CSVs ended at different abort times, and quoted the artificially-tight q/window spread instead of the seed bootstrap) — now fixed. `q={0.2,0.3}` is the reliable particle set (7/8 valid); **`q=0.1` is too noisy** at these counts; across q-sets `T_core` spans 1.23–1.33e-4 (~7%, <10%).
- **🟢 Sensitivity battery DONE (Exp. B/C/D/E) — agreement is robust, offset source pinned:** every Np=3.2e5 particle seed-bootstrap CI contains the LDG value across **q_window** (B: 0.5–0.9 all contain LDG; default 0.8 near-best, smaller windows shed v-mass), **K** (C: K=10/12/16 all ≈1.27–1.31e-4, all contain LDG — `K` is *not* the lever, CIs widen with K from MC noise), and the **LDG metric itself is robust** (E: quad order ≥5 → 1.16e-4 <1%; fit windows ~1.6%; q-set). The small ~4% residual offset is **reduced by a finer time step** (D: τ 2e-7→1e-7 shifts `T_core` 1.26e-4→1.14e-4, *toward* LDG) → attributable to mild **Lie-splitting error**, not bandwidth or window.
- **Decision:** a **genuine improvement over `t_b`** AND a **within-uncertainty cross-method agreement that is robust to `K`, `τ`, `q_window`, and the LDG quad/window choices**. The residual ~4% offset is a time-step (splitting) effect, not a reconstruction artifact. Keep defaults `K=10, τ=2e-7, q_window=0.8`; LDG metric uses quad≥5. Caveats kept honest: 4/12 LDG fits at the default windows (8/9 with cleaner late windows; secondary `T_L2`/`T_peak` confirm), weak `q=0.1`, wide particle CIs. Still use the **§11 limited wording** — report `T_core` as a reconstruction-light concentration-time diagnostic consistent with the LDG reference, **no** continuum blow-up time. Strong candidate for **§5.5 supporting diagnostic**.
- **Sensitivity data:** `reference_results/keller_segel_ldg_pp/{core_collapse_qwindow_20260615_0512_5a219ba/ (B),core_collapse_KD_20260615_0944_cd6ad21/ (C,D),core_collapse_equad_20260615_0944_cd6ad21/ (E1 quad)}`; `experiments/keller_segel/core_collapse_time/{analyze_param_sensitivity.py,analyze_qwindow_sensitivity.py,run_{qwindow_sensitivity,K_tau_pilot,K_tau_3e5,ldg_quad_sensitivity}.sb}`, `fit_core_collapse.py --windows`.
- **Data:** `experiments/keller_segel/core_collapse_time/{core_radii.py,fit_core_collapse.py (common-grid seeds + bootstrap + q-sets),plot_core_collapse.py,run_ldg_core_radii.sb,run_particle_core_radii.sb}`, `ldg_reference/run_ldg.py --core_radii`, `ldg_comparison/simulation.py` (qs incl. R_0.05,R_0.3), `reference_results/keller_segel_ldg_pp/core_collapse_20260615_0053_af781af/` (ldg/particle R_q CSVs + `core_fit_{all,summary,bootstrap}` + 5 figures + README).
