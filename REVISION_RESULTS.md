...
- **Decision — Scenario C (final):** the blob is the **best-behaved solver-level local operator** — most drift-stable, smoothest *real* solver drift, no spectral Gibbs — and the corrected metric shows a real (if **modest**) stability gain over both global-`K` and the spectral hybrid. But, like the spectral taper, it does **not** demonstrably improve the pre-singular tracked concentration (Q3 within seed noise; the stable setting barely corrects `v_lo`). So it remains a **reconstruction/diagnostic option, not a validated accuracy improvement** — consistent with §5.5 (core radii reconstruction-free; reconstructed peak/L2 bandwidth-sensitive). **Not in paper.** Do not claim the solver resolves near-blow-up dynamics.
- **Data:** `experiments/keller_segel/ldg_comparison/{hybrid_vfield.py,blob_residual_vfield.py (FD-verified),test_blob_residual_vfield.py,analyze_solver_field.py,analyze_taper_sweep.py,analyze_blob_sweep.py,plot_taper_sweep.py,plot_blob_sweep.py,run_solver_field_blob_sweep.sb}`, `simulation.py --solver_field {current_fourier,two_level_spectral_residual,two_level_blob_residual}` (now logs `drift_cfl_solver_field` + `drift_cfl_fourier_diag` + `abort_diagnostics.json`), `reference_results/keller_segel_ldg_pp/{solver_field_sweep_<run_id>/,sf_taper_20260614_1983_3c6ae64/,sf_blob_20260614_2010_84e6111/}` (diag CSVs + compare json + `figure_blob_sweep` + plot_data). **Not in paper.**
